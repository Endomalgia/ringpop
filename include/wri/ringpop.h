...
static const char FMT_VERSIONSTRING[16]					= "v1.0.1 Açaí\0\0\0";   // 16b Is a /0 automatically added when strings are defined like this?
...
RIEncoder* wriStartEncoder(char* ri_filepath) {
	...
	lseek(enc->fd, FMT_SIZEOF_MASTERBLOCK + FMT_SIZEOF_DACHEADER + FMT_SIZEOF_DACASSET*enc->na, SEEK_SET);
	write(enc->fd, FMT_DATABLOCKID, FMT_SIZEOF_DATAHEADER);
...
void wriWriteMaster(RIEncoder* enc) {
	enc->length = (long)MAX(flen(enc->fd), FMT_SIZEOF_MASTERBLOCK);
	...
	write(enc->fd, buf,	FMT_SIZEOF_MASTERBLOCK);
}
...
void wriWriteDAC(RIEncoder* enc) {
	...
	lseek(enc->fd, FMT_SIZEOF_MASTERBLOCK, SEEK_SET);
...
void wriAppendAssets(RIEncoder* enc, RIASSET* assets, int n) {
	...
	for (int c=enc->na-n; c<enc->na; c++) {
		...
		write(enc->fd, FMT_FILEDIVIDER, FMT_SIZEOF_FILEDIVIDER);
...
void wriReadDAC(RIEncoder* enc) {
	...
	lseek(enc->fd, FMT_SIZEOF_MASTERBLOCK + 4, SEEK_SET);
